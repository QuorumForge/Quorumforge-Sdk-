import { Networks, SorobanRpc, Contract, nativeToScVal, TransactionBuilder, BASE_FEE, scValToNative, Address } from '@stellar/stellar-sdk';
...
var BoardModule = class {
  ...
  async initializeBoard(params) {
    ...
    const tx = new TransactionBuilder(account, { fee: BASE_FEE, networkPassphrase: this.networkPassphrase }).addOperation(this.contract.call("initialize_board", membersVal, thresholdVal)).setTimeout(30).build();
...
var ProposalsModule = class {
  ...
  async mutate(method, ...args) {
    if (!this.config.keypair) throw new Error("keypair required for mutating operations");
    const account = await this.server.getAccount(this.config.keypair.publicKey());
    const tx = new TransactionBuilder(account, { fee: BASE_FEE, networkPassphrase: this.networkPassphrase }).addOperation(this.contract.call(method, ...args)).setTimeout(30).build();
    const prepared = await this.server.prepareTransaction(tx);
    prepared.sign(this.config.keypair);
    const result = await this.server.sendTransaction(prepared);
    return result.hash;
...
export { AlreadySignedError, InsufficientTreasuryError, InvalidThresholdError, NotAMemberError, ProposalAlreadyExecutedError, ProposalExpiredError, ProposalNotFoundError, QuorumForgeClient, QuorumForgeError, QuorumNotReachedError, RPC_URLS };
